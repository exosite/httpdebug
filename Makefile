...
build/linux-amd64/httpdebug: main.go
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o $@ github.com/waucka/httpdebug
# OS X Build
build/osx-amd64/httpdebug: main.go
	GOOS=darwin GOARCH=amd64 go build -o $@ github.com/waucka/httpdebug
# Windows Build
build/win-amd64/httpdebug.exe: main.go
	GOOS=windows GOARCH=amd64 go build -o $@ github.com/waucka/httpdebug
